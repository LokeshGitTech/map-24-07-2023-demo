...
const Map = () => {
    ...
    console.log("singleCountryMarker", singleCountryMarker);
    ...
    console.log("setVillaInModal", villaInModal);
    ...
    const currentItems = villasInfo && villasInfo?.slice(indexOfFirstItem, indexOfLastItem);
    ...
    const continentFunction = () => {
        ...
        mapRef.current.on("mousemove", "continent-fills", (e) => {
            ...
            if (e.features.length > 0) {
                if (hoveredId) {
                    mapRef.current.setFeatureState({ source: "continents", id: hoveredId }, { hover: false });
                }
                hoveredId = e.features[0].id;
                mapRef.current.setFeatureState({ source: "continents", id: hoveredId }, { hover: true });
            }
        });
    ...
    const countryFunction = () => {
        ...
        mapRef.current.on("click", "country-fills", (e) => {
            ...
            setSingleCountryMarker(Object.entries(destinationsData).filter(
    ...
    const handleSelectCountry = (country) => {
        let final = Object.entries(destinationsData).filter(
            (i) => i[0] == country
        )
    ...
    const setActiveCountry = () => {
        ...
        if (singleCountryMarker) {
            ...
            Object.entries(singleCountryMarker)?.map(() => {
                ...
                if (singleCountryMarker) {
                    setIsVillaInfo(true)
                    villas = Object.entries(singleCountryMarker.length !== 0 && singleCountryMarker[0][1]?.regions);
                    villas?.map((villaItem) => {
                        setVillasInfo(villas)
                        const iconElement = document.createElement("div");
                        iconElement.style.cursor = "pointer";
    ...
    return (
        <>
            <div style={{ overflow: "hidden" }}>
                ...
                {isVillaInfo && (
                    <div
                        style={{ display: "flex", flexDirection: "row", width: "100%", overflowY: "hidden" }}
                    ...
                    >
                        ...
                        >
                            <div style={{ display: "flex", flexDirection: "row" }}>
                                <div style={{ color: "gray", fontWeight: "bold" }}>Country &nbsp;  </div>
                                <div style={{ color: "#133C73", fontWeight: "bold" }}>{displayName} &nbsp;</div>
                                <div style={{ color: "gray", fontWeight: "bold" }}>{areaText} &nbsp;</div>
                            ...
                            <div>
                                <div>
                                    From the fragrant lavender fields of Provence and the sun-kissed villas of the
                                    Côte d'Azur to storybook alpine chalets, ornate Parisian retreats, and historic
                                    vineyard châteaux, find your haven in France and live in a dream...
                                </div>
                            </div>
                            <div
                                style={{
                            ...
                            >
                                <div style={{ color: "#133C73", fontWeight: "bold", marginTop: "3%" }}>
                                    Top Destination
                                </div>
                                ...
                                {currentItems?.map((villaData, index) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            key={index}
                                            className={` ${isHighlighted ? "des-btn-active" : "des-btn"}`}
                                        >
                                            {villaData[0]}
                                        </div>
                                    );
